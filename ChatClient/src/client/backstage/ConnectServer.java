...
public class ConnectServer implements Runnable{
	...
	public ConnectServer(Manager cm,Client_Frame cf){
		this.cm = cm;
		this.cf = cf;
	}
	...
	public void run(){
		...
		while(true){
			//不停的接收信息
			ChatMessage mess = cm.ReciveMessage();
			if(cm.IsConnect()){
				//根据不同的信息类型处理信息
				if(mess!=null){
					...
					}else if(mess.getMessageType().equals(MessageCode.Send_Online)){
						//如果提示更新在线用户
						//获得在线用户
					...
					}else if(mess.getMessageType().equals(MessageCode.System_Messages)){
						//如果提示是系统消息
						//根据获得者获得群聊窗口
